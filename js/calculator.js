...
export class Calculator {
  ...
  twoDatesSeconds(a,b) {
    let date1 = moment(a);
    let date2 = moment(b);
    let result = date1.diff(date2, 'seconds');
    return result;
  }

  yearOnMercury(age) {
    let mercuryAge = (Math.floor(age / .241));
    return mercuryAge;
  }

  yearOnVenus(age) {
    let venusAge = (Math.floor(age / .615));
    return venusAge;
  }

  yearOnMars(age) {
    let marsAge = (Math.floor(age / 1.881));
    return marsAge;
  }
...
yearsOnPlanet(age,sex,continent,planet) {
